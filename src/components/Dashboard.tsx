...
export default function Dashboard({ initialTab }: { initialTab?: 'bots' | 'management' | 'operation' }) {
  ...
  const [runPanelChannel, setRunPanelChannel] = useState<string>('');
  const [quickChannelId, setQuickChannelId] = useState<string>('');
  ...
  const handleDeployRunPanel = async () => {
    if (!selectedInstance || !runPanelChannel) return;
    setLoading(true);
    try {
      await safeJsonFetch('/api/instances/update', {
        method: 'POST',
      ...
      const data = await safeJsonFetch('/api/deploy-run-panel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ instanceId: selectedInstance.id, channelId: runPanelChannel })
      });
      setMessage({ text: data.message, type: 'success' });
    } catch (err: any) {
      setMessage({ text: err.message, type: 'error' });
              ...
              {dashboardTab === 'bots' ? (
                <div className="space-y-12">
                   ...
                   {(isAdmin || isStaff) && !ownerIdFilter ? (
                    ...
                    {selectedInstance && selectedInstance.status === 'متصل' && (
                      <div className="bg-zinc-900/40 border border-emerald-500/20 rounded-[3rem] p-8 space-y-6 text-right mt-8" dir="rtl">
                        <div className="space-y-1">
                          <h3 className="text-lg font-black text-white flex items-center gap-2">
                            <Send size={18} className="text-emerald-400" />
                            إرسال لوحة التشغيل السريع
                          </h3>
                          <p className="text-zinc-500 font-bold text-xs">أرسل لوحة التشغيل مباشرة دون الدخول لغرفة العمليات. اختر طريقة الإرسال المناسبة.</p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {/* Channel ID Mode */}
                          <div className="bg-black/40 border border-zinc-800/50 rounded-2xl p-5 space-y-3">
                            <div className="flex items-center gap-2 mb-1">
                              <div className="w-2 h-2 rounded-full bg-blue-400" />
                              <span className="text-xs font-black text-zinc-400 uppercase tracking-wider">إرسال عبر بوت</span>
                            </div>
                            <input
                              type="text"
                              ...
                              className="w-full bg-black/50 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none transition-all text-white font-mono text-left"
                              dir="ltr"
                            />
                            <button
                              onClick={async () => {
                                if (!quickChannelId || quickDeployLoading) return;
                                setQuickDeployLoading(true);
                                try {
                                  const data = await safeJsonFetch('/api/deploy-run-panel', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ instanceId: selectedInstance.id, channelId: quickChannelId })
                                  });
                                  setMessage({ text: data.message, type: 'success' });
                                  setQuickChannelId('');
                                } catch (err: any) {
                              ...
                              disabled={!quickChannelId || quickDeployLoading}
                              className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-30 text-white py-2.5 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2"
                            >
                              <Send size={12} />
                              إرسال للقناة
                            </button>
                          ...
                          {/* Webhook Mode */}
                          <div className="bg-black/40 border border-emerald-500/20 rounded-2xl p-5 space-y-3">
                            <div className="flex items-center gap-2 mb-1">
                              <div className="w-2 h-2 rounded-full bg-emerald-400" />
                              <span className="text-xs font-black text-zinc-400 uppercase tracking-wider">إرسال عبر ويب هوك</span>
                            </div>
                            <input
                              type="text"
                              ...
                              className="w-full bg-black/50 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-white font-mono text-left truncate"
                              dir="ltr"
                            />
                            <button
                              onClick={async () => {
                                if (!quickWebhookUrl || quickDeployLoading) return;
                                setQuickDeployLoading(true);
                                try {
                                  const data = await safeJsonFetch('/api/deploy-run-webhook', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ webhookUrl: quickWebhookUrl })
                                  });
                                  setMessage({ text: data.message, type: 'success' });
                                  setQuickWebhookUrl('');
                                } catch (err: any) {
                              ...
                              disabled={!quickWebhookUrl || quickDeployLoading}
                              className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-30 text-white py-2.5 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2"
                            >
                              <Send size={12} />
                              إرسال عبر الويب هوك
                            </button>
                ...
                !isOperationsVerified ? (
                  ...
                  <div className="flex flex-col gap-12">
                     <div className="space-y-16">
                      ...
                      {/* Run Panel Deployment */}
                      <div className="bg-zinc-900/30 border border-zinc-800 rounded-[3rem] p-12 space-y-8">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-zinc-800/50">
                          <div className="space-y-1 text-right">
                            <div className="flex items-center gap-3 mb-2 justify-start">
                              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                              <span className="text-emerald-400 font-black text-xs uppercase tracking-widest">Quick Deploy</span>
                            </div>
                            <h3 className="text-3xl font-black text-white tracking-tight">لوحة التشغيل السريع</h3>
                            <p className="text-sm font-bold text-zinc-500">إرسال لوحة تشغيل البوت في قناة محددة. عند الضغط على الزر،سيتم طلب التوكن ومعرف المستخدم من العضو.</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <select
                              value={runPanelChannel}
                              onChange={(e) => setRunPanelChannel(e.target.value)}
                              className="bg-black border border-zinc-800 rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-[#c5a059] outline-none transition-all appearance-none cursor-pointer text-white"
                            >
                              <option value="">اختر القناة...</option>
                              {discordData.channels.filter(c => c.type === 0).map(c => (
                                <option key={c.id} value={c.id}>{c.name}</option>
                              ))}
                              {!discordData.channels.length && runPanelChannel && (
                                <option value={runPanelChannel}>{runPanelChannel}</option>
                              )}
                            </select>
                            <button
                              onClick={handleDeployRunPanel}
                              disabled={!runPanelChannel || loading || selectedInstance.status !== 'متصل'}
                              className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3.5 rounded-2xl font-black text-sm transition-all shadow-xl shadow-emerald-600/20 active:scale-95 disabled:opacity-30 flex items-center gap-2"
                            >
                              <Send size={16} />
                              إرسال لوحة التشغيل
                            </button>
                        ...
                        <div className="bg-black/40 border border-zinc-800/50 rounded-2xl p-6">
                          <p className="text-xs text-zinc-500 font-bold leading-relaxed text-right" dir="rtl">
                            كيف يعمل؟ Members في السيرفر يضغطون على زر "تشغيل البوت" → تظهر لهم نافذة لإدخال التوكن ومعرف الدسكورد → ينشئ البوت تلقائياً ويرسل لهم رابط لوحة التحكم عبر الرسائل الخاصة (DM).
                          </p>
                        </div>
                      </div>
                    </div>
